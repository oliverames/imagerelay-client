...
- **Live testing scope**: Signed Developer ID builds have been smoke-tested against the live Image Relay account only inside the selected `Test Library` folder (`12345`). The stable `1.4.4` / build `47` release was published on 2026-08-26 after release-candidate checks, Developer ID notarization, GitHub release publishing, published-DMG SHA verification, and Gatekeeper, staple, and Sparkle-signature checks on the downloaded DMG; no smoke install was run, so the installed-app path is unverified for `1.4.4`. The prior `1.4.3` / build `46` release (2026-06-10) was smoke-installed. The last full live sync matrix remains the `1.4.0` / build `43` matrix in `Test Library` from 2026-06-02. Keep future release testing constrained to that folder unless explicitly asked otherwise.
