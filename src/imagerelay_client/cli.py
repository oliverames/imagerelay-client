...
from .appdirs import database_path
from .api import ImageRelayApiClient, ImageRelayApiError
from .autostart import AutoStart, AutoStartError
from .config import ConfigStore, Settings, SyncConfigurationError, mask_api_key
...
def _api_click_error(error: Exception) -> click.ClickException:
    return click.ClickException(user_message_for_error(error))
...
def init(
    ...
    if remote_root_folder_id is not None:
        ...
        try:
            ...
            raise _api_click_error(exc) from exc
        settings.remote_root_folder_id = root.folder_id
        click.echo(f"Detected Image Relay root folder: {root.folder_id}")

    store.save(settings)
    click.echo(f"Saved config to {store.path}")
    ...
    if settings.user_agent == Settings().user_agent:
        click.echo(
            "Warning: update `user_agent` with your real project URL or contact email to satisfy Image Relay API requirements."
        )
...
def config_set(key: str, value: str) -> None:
    ...
    if key == "api_key":
        ...
        saved_value = "(cleared)"
    click.echo(f"Updated {key}: {saved_value}")
...
@sync.command("once")
@click.pass_context
def sync_once(ctx: click.Context) -> None:
    try:
        run_sync_once(verbose=ctx.obj["verbose"])
    except SyncConfigurationError as exc:
        raise click.ClickException(str(exc)) from exc
    except SyncPausedError as exc:
        raise click.ClickException(user_message_for_error(exc)) from exc
    except ImageRelayApiError as exc:
        raise _api_click_error(exc) from exc
...
def sync_status(json_output: bool) -> None:
    ...
    click.echo(f"Status: {snapshot.sync_state}")
    click.echo(f"Pause: {'Active' if snapshot.paused else 'Not Paused'}")
    click.echo(f"Pause Until: {snapshot.paused_until or '--'}")
    click.echo(f"Phase: {snapshot.sync_phase}")
    click.echo(f"Progress: {snapshot.completed_steps}/{snapshot.total_steps or 0}")
    click.echo(f"ETA Seconds: {snapshot.eta_seconds if snapshot.eta_seconds is not None else '--'}")
    click.echo(f"Current Item: {snapshot.current_item or '--'}")
    click.echo(f"Last Remote Pull: {snapshot.last_remote_poll_at or '--'}")
    click.echo(f"Next Remote Pull: {snapshot.next_remote_poll_at or '--'}")
    if recent_activity:
        click.echo("Recent Activity:")
        for item in recent_activity:
            click.echo(f"  - {item['action']}: {item['rel_path']}")
    if snapshot.last_error:
        click.echo(f"Last Error: {snapshot.last_error}")
...
def autostart_enable(target: str) -> None:
    try:
        ...
        raise click.ClickException(str(exc)) from exc
    click.echo(f"Autostart enabled: {path}")
...
def autostart_status(target: str) -> None:
    ...
    if autostart.enabled:
        click.echo(f"Autostart is enabled: {autostart.path}")
    else:
