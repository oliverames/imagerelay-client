...
**Left off at**: `1.2.0-beta.2` build `31` is ready to publish from branch `feature/1.2-beta2-sync-hardening`. `swift test --package-path ImageRelayKit`, `xcodebuild test -project ImageRelayClient.xcodeproj -scheme ImageRelayClient -destination 'platform=macOS'`, and `xcodebuild build -project ImageRelayClient.xcodeproj -scheme ImageRelayClientiOS -destination 'platform=iOS Simulator,name=iPhone 17e' CODE_SIGNING_ALLOWED=NO` passed. `scripts/run-release-candidate-checks.sh 1.2.0-beta.2` passed. `scripts/build-developer-id-release.sh --version 1.2.0-beta.2 --smoke-install` passed, producing a signed, notarized, stapled DMG with SHA-256 `d9f42943b8a07fa7201241a75d104bd0c51728cb40fe09e79a897368b0833fac`. App ZIP notarization submission `514f0388-1f60-4f07-abba-ae1a63945383` and DMG notarization submission `ede977b6-46fa-4e96-9180-f554766af9dd` were accepted; the smoke install registered `com.oliverames.imagerelay-client.fileprovider(1.2.0-beta.2)`.
