...
Test count grew from 179 across 25 suites (1.3.0-beta.1) to 211 across 27 suites (1.3.0-beta.2): 163 ImageRelayKitTests (+13 — 11 `FilenamePresentation` round-trip cases + 2 `Configuration` legacy-decode/round-trip cases) + 48 FileProviderExtensionTests (+19 — 4 `FileProviderItem` display-transform cases + 15 `ActionFormatting` cases covering Markdown formatting, mailto subject/body, URL build/parse round-trip, name padding, scheme rejection, and PNG magic-number validation of QR output). Current User-Agent defaults advanced from `ImageRelayClient/1.3.0-beta.1` to `ImageRelayClient/1.3.0-beta.2` for both `(macOS)` and `(iOS)`; the 1.3.0-beta.1 User-Agents were added to `legacyMacUserAgents` and the `normalizedIOSUserAgent` switch so 1.3.0-beta.1 installs roll forward without a reset.
...
**Left off at**: `xcodebuild test -project ImageRelayClient.xcodeproj -scheme ImageRelayClient -destination 'platform=macOS'` passed 211 tests across 27 suites (163 ImageRelayKitTests + 48 FileProviderExtensionTests). `xcodebuild build -project ImageRelayClient.xcodeproj -scheme ImageRelayClientiOS -destination 'platform=iOS Simulator,name=iPhone 17e'` reported BUILD SUCCEEDED — the kit-level FilenamePresentation/AppConfiguration changes compile and link cleanly on iOS too. Per user direction, the Developer ID build (`scripts/build-developer-id-release.sh --version 1.3.0-beta.2 --smoke-install`) was deferred. The Beautify Filenames toggle and all eleven new actions are wired but have not been live-tested against the BCBSVT account; the Tier 3 deep-link routing in particular has not been exercised in the real Finder right-click → host-app flow.
