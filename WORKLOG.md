...
**Left off at**: `v1.2.1` will be tagged from the release commit on `main` after the signed/notarized build completes. `swift test --package-path ImageRelayKit` passed (144 tests across 20 suites). `xcodebuild test -project ImageRelayClient.xcodeproj -scheme ImageRelayClient -destination 'platform=macOS'` passed (168 tests across 23 suites). Next steps: run `scripts/build-developer-id-release.sh --version 1.2.1 --smoke-install`, capture the DMG SHA-256 and notarization submission IDs, sync the Homebrew cask, and post-install verify that `NSWorkspace.shared.open` actually opens the browser from the FP extension XPC sandbox (the pasteboard fallback is the safety net if it returns false).
