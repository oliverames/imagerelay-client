...
for tool in op python3 xcodegen xcodebuild xcrun hdiutil codesign spctl ditto shasum security; do
...
codesign --verify --deep --strict --verbose=4 "$APP_PATH" | tee "$ARTIFACT_DIR/codesign-exported-app.log"
...
spctl -a -vv "$APP_PATH" | tee "$ARTIFACT_DIR/spctl-app.log"
...
codesign --verify --deep --strict --verbose=4 "$DMG_ROOT/ImageRelayClient.app" | tee "$ARTIFACT_DIR/codesign-dmgroot-app.log"
...
codesign --sign "$DEVELOPER_ID_APPLICATION_IDENTITY" \
  --timestamp \
  -i "$DMG_SIGNING_ID" \
  "$DMG_PATH" | tee "$ARTIFACT_DIR/codesign-dmg-sign.log"
codesign --verify --strict --verbose=4 "$DMG_PATH" | tee "$ARTIFACT_DIR/codesign-dmg.log"
...
spctl --assess --type open --context context:primary-signature -vv "$DMG_PATH" | tee "$ARTIFACT_DIR/spctl-dmg.log"
...
if [[ "$SMOKE_INSTALL" -eq 1 ]]; then
  ...
  codesign --verify --deep --strict --verbose=4 /Applications/ImageRelayClient.app | tee "$ARTIFACT_DIR/codesign-installed-app.log"
  spctl -a -vv /Applications/ImageRelayClient.app | tee "$ARTIFACT_DIR/spctl-installed-app.log"
  ...
  for _ in {1..30}; do
    if pgrep -f "$APP_PROCESS_PATTERN" >/dev/null 2>&1 && pgrep -f "$APPEX_PROCESS_PATTERN" >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
  ...
  if ! pgrep -f "$APPEX_PROCESS_PATTERN" >/dev/null 2>&1; then
    echo "Smoke install failed: File Provider extension process did not launch." >&2
    exit 1
  fi

  /usr/bin/log show --last 5m --style compact \
    --predicate '(process == "ImageRelayClient" OR process == "FileProviderExtension" OR subsystem == "com.oliverames.imagerelay-client" OR subsystem == "com.oliverames.imagerelay-client.fileprovider")' \
